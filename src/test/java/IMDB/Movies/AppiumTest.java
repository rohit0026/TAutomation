package IMDB.Movies;
import org.testng.annotations.Test;


public class AppiumTest {



	//private static org.apache.log4j.Logger log =  LogManager.getLogger(AppiumTest.class.getName());
	//private static Logger log = LogManager.getLogger(AppiumTest.class.getName());
	

	@Test(groups = "Regression")
	public void NativeApp()
	{
		System.out.println("Native1 app from cloned changes Appium tests Part of Regression");
		//if(5>4)
		
	}
	@Test(groups = "Regression")
	public void HybridApp()
	{
		System.out.println("Hybrid app from Appium tests Part of Regression");
		System.out.println("changes from GIT");
	}

}
